...
void CExcelHPE::SetColumnWidth(int column, double width)
{
	CRange range;
	range.AttachDispatch(excel_work_sheet_.get_Columns(), true);
	//设置第一列的列宽  
	CRange cols;
	cols.AttachDispatch(range.get_Item(COleVariant((long)column), vtMissing).pdispVal, TRUE);
	cols.put_ColumnWidth(COleVariant(width)); //设置列宽
}
