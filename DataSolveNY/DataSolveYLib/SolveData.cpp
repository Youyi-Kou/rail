...
{
	int         useL = 0;   //该米块在文件中的字节位置
	BLOCK_A		head;   //tpA米块头
	uint32_t	iBlockRead = 0;
	uint32_t    s_FileL_A = (iFileSizeA - szFileHead) >> 1;   //右移一位
	uint32_t    iTotalBlock = vBlockHeads.size();
	for (int i = 0; i < iTotalBlock; ++i)
	{
		vBlockHeads[i].AStartPos = -1;   //A超起始位置点
		vBlockHeads[i].FrameCount = 0;   //米块头指示的A超帧数
		vBlockHeads[i].FrameCountRead = 0;   //读到的A超帧数
	}
	while (useL < s_FileL_A && iBlockRead < iTotalBlock)
	{
		...
		if (head.symbol != 0xFFFFFFFF)   // 米块标志
		{
			useL++;
			continue;
		}
		else if (head.indexL != vBlockHeads[iBlockRead].IndexL)   //tpA米块头起始步进 不等于 tpB米快初始步进
		{
