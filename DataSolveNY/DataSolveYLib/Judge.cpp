...
void    SetFileHead(F_HEAD& head)   // 设置文件头
{
	...
	vExistedPMs.clear();   //人工标记
	g_vJointPos.clear();   //接头位置点
	g_vSewPos.clear();   //焊缝位置点

	g_vReturnSteps.clear();   //回退步进
	g_vFLoseBig.clear();   //出波连通域
	...
	g_vStepsInBackArea.clear();   //处理道岔用
	g_vStepsInJoint.clear();   //处理接头
	g_vStepsInSew.clear();   //处理焊缝
	g_vStepsInK.clear();   //处理里程校正
	g_vStepsInCouple.clear();   //处理失耦
	g_vStepsInWound.clear();   //处理标伤
	g_vHoleParas.clear();   //孔
	g_vSlopes.clear();   //变坡点
...
void	FillMarksWounds(VBDB& blocks, VPM& vPMs, VWJ& vWounds, VPM& vPMs2, VLCP& vLCP, F_HEAD& g_fileHead)
{
	...
	for (size_t i = 0; i < blocks.size(); i++)
	{
		for (size_t j = 0; j < blocks[i].vBStepDatas.size(); j++)
		{
			...
			if (wound & W_MAN)
			{
				...
				{
					Wound_Judged wd;   //伤损数据结构体Wound_Judged 变量wd
					...
					wd.According.emplace_back(wd.Result);   //推入数据伤损数据结构体

					FillWound2(wd, blocks);   //Curve曲线伤损、Bridge桥梁伤损
					...
					AddToWounds(vWounds, wd);   //添加伤损信息 伤损数据结构体Wound_Judged 变量wd
					g_vStepsInWound[blocks[i].IndexL2 + j] = 1;   //处理人工标记伤损标签
				}
			}
		}
	}
	...
	for (int i = 1; i < blocks.size(); ++i)
	{
		if (Abs(blocks[i].FRow2 - blocks[i - 1].FRow) >= 3)
		{
			...
			pm.Mark = PM_SMART1;   //PM_SMART1 尖轨变坡点
...
{
#pragma region 判定厂焊出波的大小通道分割线
	int iStepSew2 = iStepSew;
	iStepSew = GetSewCHMiddleStep(blocks, vCRs, cr, t_cr, iAStepBig, iAStepSmall);   //寻找厂焊大小通道中间分开的步进
	if (iStepSew < 10)
	{
		iStepSew = iStepSew2;
	}
	...
	//	iStepSew = iStepSew2;
	//}
	Pos	 pos = FindStepInBlock(iStepSew, g_vBlockHeads, cr.Block - g_iBeginBlock);   //寻找某个步进的位置
...
#pragma region 检测核伤
	for (int j = 0; j < 10; ++j)
	{
		...
		for (int k = 0; k < t_cr[j].size(); ++k)
		{
			...
			if (j % 4 == 0)//大通道
			{
				GetCRInfo(t, DataA, blocks, t_cr[j].size() == 1);   //每个通道只能有一个主出波，检查多余的出波
				//正常位置出波
				if (t.Step1 >= iStepSew)
				{
					//A, B
...
#pragma region 再次检验焊缝附近是否有DEFG通道出波 失波
	crD.clear();
	crE.clear();
...
#pragma region 检测接头中部非螺孔出波的DEFG通道
...
#pragma region 检测核伤（按核伤判伤标准判定） ，并检测缺失回波的通道
	uint8_t isSew = 2;
	wound.IsSew = isSew;
	wound.Type = W_HS;
	wound.Block = cr.Block;
...
#pragma region 检测焊缝处非螺孔出波的DEFG通道
	crD.clear();
	crE.clear();
...
#pragma region 铝热焊轨底端角反射(将轨底的正常端角反射出波判定为正常波)
	crD.clear(); crE.clear();
	bFindD = GetCR(CH_D, iStepSew2 -10, iFRow - 2, iStepSew2 + 10, iFRow + 10, blocks, vCRs[CH_D], crD);
	bFindE = GetCR(CH_E, iStepSew2 - 10, iFRow - 2, iStepSew2 +10, iFRow + 10, blocks, vCRs[CH_E], crE);
...
#pragma region 检测核伤（每个通道只能有一个主出波，出波两个及以上判伤） ，并检测缺失回波的通道
	for (int j = 0; j < 10; ++j)
	{
		int t_A = GetAChannelByBChannel(j);
...
#pragma region 现场焊轨底端角反射(将轨底的正常端角反射出波判定为正常波)
	crD.clear(); crE.clear();
	bFindD = GetCR(CH_D, iStepSew2 - 10, iFRow - 2, iStepSew2 + 10, iFRow + 10, blocks, vCRs[CH_D], crD);
	bFindE = GetCR(CH_E, iStepSew2 - 10, iFRow - 2, iStepSew2 + 10, iFRow + 10, blocks, vCRs[CH_E], crE);
...
#pragma region 螺孔伤损判定
	...
	int pIndex = GetManualJoint(stepFRight - 100, vPMs);   //在失波步进-100，到失波结束步进之间是否存在仪器标记的#
	if (pIndex >= 0)
	{
		bAutoJoint = true;
	}
	...
	if (crFLose.size() == 1 && crGLose.size() == 1)
	{
		...
		if (sl - sr < 30)   //寻找FG通道的失波位置，若失波距离小于30步进，则修改失波步进为该步进
		{
			stepFRight = sr;
			stepFLeft = sl;
#ifdef _DEBUG
	...
	//在指定的步进位置上判定可能存在的6个螺孔
	bool isExistScrewHoles[6];
	uint8_t count = 0;

	int oldRT = railType;
	posRight = FindStepInBlock(stepFRight - 20, blocks, 0);   //将接头左侧右侧两侧螺孔分开判断，因为可能接头两侧的钢轨轨型不一致
	uint8_t fRow = iFRow;
	...
	posLeft = FindStepInBlock(stepFLeft + 20, blocks, 0);   //将接头左侧右侧两侧螺孔分开判断，因为可能接头两侧的钢轨轨型不一致
	fRow = iFRow;
...
#pragma region 先检测是否有核伤（大小通道位置相反 偏离正常出波位置）
	for (int i = 0; i < CH_C; ++i)
	{
		for (int j = 0; j < t_cr[i].size(); ++j)
		{
...
#pragma region 再次检验接头两侧是否有倒打螺孔斜裂纹
...
bool ParseHS(BlockData_A& DataA, VBDB& blocks, VCR* vCRs, CR& cr, CR_INFO_A& crInfo, uint8_t& railType, bool& direction, bool& carType, int16_t& iFRow, double& wd, VWJ& vWounds, VPM& vPMs, uint8_t isJoint, uint8_t isSew, uint8_t isScrewHole, uint8_t isGuideHole)
{
	...
	if (m == CH_A1 || m == CH_a1 || m == CH_B1 || m == CH_b1)
	{
		...
		if (isJoint == 0 && isSew == 0)
		{
			...
			if (cr.Row1 > g_iJawRow[railType])//出现二次波，找对应一次波
			{
...
#pragma endregion				
	...
	else if (m == CH_C || m == CH_c)   //C, c 通道
	{
		...
		if ((crInfo.Shift >= 15 || cr.Region.size() >= 3) && (crInfo.MaxV >= hsAmpl || cr.IsContainA == 0) && cr.Row1 < g_iJawRow[railType] * 2 || cr.Row2 - cr.Row1 >= 7 || crInfo.Shift >= 50 && cr.Region.size() >= 5)
		{
			...
			if (isScrewHole || isGuideHole || bScrew || bGuide)  //判定是否螺孔、导孔上方回波
			{
				...
				if (isScrewHole || bScrew)
				{
					if (cr.Row1 >= iLastScrewHoleRow - 8 && cr.Row2 <= iLastScrewHoleRow2 + 3)   //判断出波高度和螺孔的高度是否匹配
					{
				...
				{
					if (cr.Row1 >= iLastGuideHoleRow - 5 && cr.Row2 <= iLastGuideHoleRow2 + 3)   //判断出波高度和导孔的高度是否匹配
					{
...
int SolvePosition(uint16_t parsedMark, BlockData_A& DataA, VBDB& blocks, VCR* vCRs, CR& cr, int i, double wd, int16_t iFRow, uint8_t railType, bool carType, bool direction, VINT* t_cr, PM& pm, int iAStepBig, int iAStepSmall, VPM& vPMs, VPM& vPMs2, VWJ& vWounds, uint8_t isPreAnalyse)
{
	...
	else if (pm.Mark == PM_SEW_LRH)   //铝热焊判伤
	{
		iAStepBig += 5;
		iAStepSmall -= 5;
		int istepSew = GetSewLRHMiddleStep(blocks, vCRs, cr, t_cr, iAStepBig, iAStepSmall, pm);   //判定铝热焊出波的大小通道分割线
		if (iAStepBig < 20)
		{
			istepSew = cr.Step2;
		}
		pm.MiddleStep = istepSew;
		Pos	 pos = FindStepInBlock(istepSew, g_vBlockHeads, 0);   //寻找某个步进的位置
		pm.Block = pos.Block;
		pm.Step = pos.Step;
		pm.Step2 = pos.Step2;
	...
	else if (pm.Mark == PM_SEW_LIVE)   //现场焊判伤
	{
		iAStepBig += 5;
		iAStepSmall -= 5;
		int istepSew = GetSewXCHMiddleStep(blocks, vCRs, cr, t_cr, iAStepBig, iAStepSmall);   //寻找现厂焊大小通道中间分开的步进
		if (iAStepBig < 20)
		{
			istepSew = cr.Step2;
		}
...
void IsHole(BlockData_A& DataA, VBDB& blocks, VCR* vCRs, CR& cr, int i, double wd, int& offset, double angle, uint8_t& railType, int16_t& iFRow, VPM& vPMs, bool& bGuideHole, bool& bScrewHole, int &iFStep_1, int &iFStep_2, VINT& crD, VINT& crF, VINT& crG, VINT& crE, VINT& crF2, VINT& crG2, VWJ& vWounds)
{
	...
	if (cr.Region.size() < 5)
	{
		...
		if (vcr.size() >= 20)
		{
			return;   //若出波连通域点数小于5 前后300步进内，该通道出波连通域个数不少于20，则该处出波为杂波
		}
	}
	...
	if (cr.Row1 >= iJawRow + 5 && cr.Row1 <= iLuokong_D_Row1_H[railType] - wear && cr.Row1 >= iLuokong_D_Row1_L[railType] - wear && (bFindE || bFindF || bFindG) && (bLoseF && bLoseG))//螺孔高度出D波
	{
		...
		{
			pm.Mark = PM_SCREWHOLE;   //判定为螺孔
			//AddToMarks(pm, vPMs);
			bScrewHole = true;
		}
		else
		{
			pm.Mark = PM_GUIDEHOLE;   //判定为导孔
			//AddToMarks(pm, vPMs);
			bGuideHole = true;
		}
	}
	...
	if (cr.Row1 >= iJawRow + 6 && cr.Row2 <= iFRow - 10 && crD.size() > 0)
	{
		...
		for (int k = 0; k < crD.size(); ++k)
		{
			CR& tpD = vCRs[CH_D][crD[k]];   //遍历所有D通道出波，记为tpD
			//若tpD与cr出波高度差大于10或tpD出波方向不正确，不为孔
			...
			if (iCountB >= 0.5 * tpD.Region.size())
			{
				...
				else if (bFindE == false && bFindF == false && bFindG == false)
				{
					...
					if (dv >= 4 && dv <= 10)
					{
						...
						memset(&pm, 0, sizeof(pm));
						pm.Walk = wd;
						pm.Mark = PM_SCREWHOLE;   //上方的为正常螺孔出波
						...
						w.Walk = GetWD(blocks[w.Block - g_iBeginBlock].BlockHead.walk, w.Step, g_filehead.step, g_direction);
						w.Degree = WD_SERIOUS;
						sprintf(w.Result, "%d孔四象限斜裂纹", 0);   //靠下方的出波为螺孔四象限斜裂纹
						w.Type = W_SCREW_CRACK4;
						w.Place = WP_WAIST;
					...
					else if (dv < 4)
					{
						...
						memset(&pm, 0, sizeof(pm));
						pm.Walk = wd;
						pm.Mark = PM_GUIDEHOLE;   //上方的为正常导孔出波
						pm.Block = crtemp.Block;
						pm.Step = crtemp.Step;
						pm.Step2 = crtemp.Step2;
						AddToMarks(pm, vPMs);
						...
						w.Walk = GetWD(blocks[w.Block - g_iBeginBlock].BlockHead.walk, w.Step, g_filehead.step, g_direction);
						w.Degree = WD_SERIOUS;
						sprintf(w.Result, "导孔四象限斜裂纹");   //靠下方的出波为导孔四象限斜裂纹
			...
			else if (iCountA >= 0.5 * tpD.Region.size() && bFindE == false && bFindF == false && bFindG == false)
			{
				...
				memset(&pm, 0, sizeof(pm));
				pm.Walk = wd;
				pm.Mark = PM_GUIDEHOLE;   //判定为导孔，此处为导孔双波
				pm.Block = cr.Block;
				pm.Step = cr.Step;
				pm.Step2 = cr.Step2;
				AddToMarks(pm, vPMs);
	...
	//S1.6 校验与接头焊缝位置
	uint32_t sew, joint;
	int dt1 = GetLastPos(cr.Step1, g_vSewPos, sew);   //与焊缝距离
	int dt2 = GetLastPos(cr.Step1, g_vJointPos, joint);   //与接头距离

	if (dt1 <= 30 || dt2 <= 100)
	{
		bGuideHole = false;   //判定为非导孔
		return;
	}
}
...
void ParseD(double angle, BLOCK& blockHead, BlockData_A& DataA, VBDB& blocks, VCR* vCRs, CR& cr, CR_INFO_A& crInfo, int16_t& iFRow, int i, int& offset, double& wd, uint8_t& railType, VWJ& vWounds, VPM& vPMs)
{
	...
	else
	{
		...
		int s1 = 0, s2 = 0;
		GetCRASteps(CH_D, cr, s1, s2, blocks, angle, offset, g_filehead.step);   //用于判断螺孔、导孔的双波
		VINT crDD;
...
void AnalyseCR(BlockData_A& DataA, VBDB& blocks, VBA& vBA, VCR* vCRs, CR& cr, int& i, VWJ& vWounds, VPM& vPMs, VER& vER, VPM& vPMs2, VLCP& vLCP, int btIndex)
{
	...
	int offset = g_filehead.deviceP2.Place[iChA] + blockHead.probOff[iChA];   // 探头位置+探头位置偏移
	double angle = 0.1 * g_filehead.deviceP2.Angle[iChA].Refrac;   // 仪器参数.探头角度及偏角.折射角
	...
	{
		VINT vu;
		GetCR(cr.Channel, cr.Step1 - 100, cr.Row1, cr.Step2 + 100, cr.Row2, blocks, vCRs[cr.Channel], vu, i, 1);   //函数GetCR()
	...
	bool bFindFrames = GetCRInfo(cr, DataA, blocks, isUnique);   //函数GetCRInfo()
	CR_INFO_A& crInfo = cr.Info;
	std::vector<A_Step>& vASteps = cr.vASteps;
	...
	bool carType = blockHead.detectSet.Identify & BIT2;   // 车型，1-右手车，0-左手车
	double wd = GetWD(blockHead.walk, cr.Step, g_filehead.step, direction);   //方向？？？
	...
	if (m < 14)
	{
		...
		{
			ParseHS(DataA, blocks, vCRs, cr, crInfo, railType, direction, carType, iFRow, wd, vWounds, vPMs);//核伤
		}
		else if (m == CH_D)//D通道
		{
...
{
	Pos pos = FindStepInBlock(step, g_vBlockHeads, 0);   //寻找某个步进的位置
	...
	{
		if (GetCR(j, step - stepLimit, g_iJawRow[railType] - 2, step + stepLimit, g_iJawRow[railType] + 2, blocks, vCRs[j], t_cr[j]))   //判定并获取连通域t_cr
		{
			int iMaxIndex = 0;
...
void Analyse(BlockData_A& DataA, VBDB& blocks, VBA& vBA, VWJ& vWounds, VPM& vPMs, VER& vER, VPM& vPMs2, VLCP& vLCP, VPM& vPMsYOLO, int backFlag/* = 0 */, int btIndex/* = -1 */)
{
	...
	int iEndBlock = blocks[blocks.size() - 1].Index;   //米块索引，即第几个米块
	//vReturnSteps.clear();
	...
	int markCount = vPMs.size();   //检出位置标数量
	int woundCount = vWounds.size();   //检出伤损数量
	//S2 获取尖轨的位置标记
	FillMarksWounds(blocks, vPMs, vWounds, vPMs2, vLCP, g_filehead);   //提取人工标记伤损：AddToWounds()添加到伤损数据结构体中Wound_Judged  提取人工标记位置标数据：添加到位置标数据结构体中Position_Mark

	//S3 本期数据判伤，求连通域
	//A a B b C c D d E e F G
	//需记录 A,a, B, b,C,c, D, E, F, G的出波
	VPM vSmarts;   //位置标：vSmarts变坡点
	for (int i = markCount; i < vPMs.size(); ++i)
	{
		if (vPMs[i].Mark == PM_SMART1 || vPMs[i].Mark == PM_SMART2)   //PM_SMART1尖轨变坡点  PM_SMART2异型轨变坡点
		{
	...
	g_iBeginStep = blocks[0].IndexL2;   //米快初始步进
	...
	VCR vCRs[16];   //class&vector 出波连通域
	//统计各通道出波点
	CreateCR(blocks, vCRs, vER, vSmarts);

#pragma region 获取大块FG失波，提取锰钢岔（FG同时失波在500步进以上，需要与上一个分段的末尾联合判断）

	//找出FG同时失波区域 vFLose
	int iLastLFCount = g_vFLoseBig.size();

	VCR vFLose;   //出波连通域结构体  此处为连续失波
	for (int i = 0; i < vCRs[CH_F].size(); ++i)
	{
		...
		if (cF.Step2 - cF.Step1 >= 100 || cF.IsStart || cF.IsEnd)
		{
			...
			{
				VINT crF;
				GetCR(CH_F, sF2 - 5, cF.Row1 - 5, sF2 + 10, cF.Row2 + 5, blocks, vCRs[CH_F], crF, vExcept, 5);   //找满足条件的轨底失波连通域crF 行[-5,5] 步进[-5,10]
				RemoveCRByLose(vCRs[CH_F], crF, 1);   //去除误判
			...
			{
				VINT crF;
				GetCR(CH_F, sF1 - 50, cF.Row1 - 5, sF1 + 10, cF.Row2 + 5, blocks, vCRs[CH_F], crF, vExcept, 5);   //找满足条件的轨底失波连通域crF 行[-5,5] 步进[-50,10]
				RemoveCRByLose(vCRs[CH_F], crF, 1);   //去除误判
			...
			for (int j = 0; j < crG.size(); ++j)
			{
				...
				int bs, es;
				int os = GetOverlappedStep(sF1, sF2, sG1, sG2, bs, es);   //重叠区域步进差
				if (os >= loseLenLimit - 10)
				{
					CR cLose;
					cLose.Channel = CH_F;
					cLose.Step1 = bs;   //重叠区域开始步进
					cLose.Step2 = es;   //重叠区域结束步进
					cLose.IsStart = vCRs[CH_F][i].IsStart;
					cLose.IsEnd = vCRs[CH_F][i].IsEnd;
					Pos pos = FindStepInBlock(bs, blocks, 0);   //寻找某个步进的位置
					cLose.Block = pos.Block;
					cLose.Step = pos.Step;
					for (int k = bs; k <= es; ++k)
					{
						WaveData wd;   //B超出失波记录结构体
						wd.block = pos.Block;
						wd.step = k;
						wd.row = cF.Row1;
						wd.find = 0;
					...
					FillCR(cLose);   //计算连通域参数
					vFLose.emplace_back(cLose);   //出波连通域结构体 此处为连续失波
				}
			}
		}
	}
...
#pragma region 获取FG底波变化点（两段不同高度之间的...）
	FRowRecord lastFR;  //Step、FRow、GRow
	if (g_vFR.size() > 0)
	{
...
#pragma region 多出波点区间统计
	...
	CreateSection(vWeight, 100, vsec);   //出波区间统计，组成一个个Section 遍历-相邻两个点之间的步进不大于100-则划为同一个section
	CreateSection(vWeight2, 50, vsec2);
	CreateSection(vWeight3, 50, vsec3);

	std::vector<Pos> vPosMarks;   //位置点
	std::vector<Section> vHeavySteps;   //初始定义使用出波区间结构体 但后续定义更改 vHeavySteps[i].Start改为区域Section内轨头出波点数最大步进 vHeavySteps[i].End改为轨头15步进出波点数统计
	Section sec;
	...
	for (int i = 0; i < vsec.size(); ++i)
	{
		...
		int maxIndex = vsec[i].Start - g_iBeginStep;   //出波区间初始步进 - 米快初始步进  Section只有开始和结束步进，这里减去米块初始步进呢和g_StepPoints()有关，CRHPE.cpp中g_stepCount = g_iEndStep - g_iBeginStep + 1保证了步进索引唯一，所有米块的步进连续
		int maxValue = g_StepPoints[maxIndex].TotalHead;   //每步进出波信息统计.轨头出波总数
		for (int j = vsec[i].Start + 1 - g_iBeginStep; j <= vsec[i].End - g_iBeginStep; ++j)  //找区域Section内轨头出波点数最大步进
		{
		...
		sec.Start = step;   //Start定义变化，区域Section内轨头出波点数最大步进
		sec.End = g_StepPoints[step - g_iBeginStep].SumHead15;   //End定义变化，15步进内出波点数
		if (sec.End >= 40)
		{
			sec.Flag = 0;
...
#pragma region 处理人工标记的接头
	...
	for (int i = markCount; i < pmCount; ++i)
	{
		...
		{
			int dstep = vPMs[i].Step2 - vHeavySteps[j].Start;   //vPMs.Step2位置标数据米块初始步进 - Section.Start
			if (dstep >= -100 && dstep <= 100)  
			{
				vHeavyIndexes.push_back(j);   //在Vector最后添加一个元素
			}
			if (dstep > 100)
			{
				heaveIndex = (j == vHeavySteps.size() - 1 ? j : j + 1);   //TODO ？？？
				break;
			}
		}
		if (vHeavyIndexes.size() == 1 && vHeavySteps[vHeavyIndexes[0]].Flag == 0)
		{
			int16_t mark = AnalyseStep(DataA, blocks, vBA, vCRs, vPMs[i].Step2, vWounds, vPMs, vER, vPMs2, vLCP, 30, i, btIndex);   //函数AnalyseStep()
			vHeavySteps[vHeavyIndexes[0]].Flag = 1;
			if (mark == 0)
			{
				vPMs[i].IsOverlapped = 1;   //是否被覆盖了，先标*1，再标*2，*1的标记会被记1
...
#pragma region 处理人工标记的铝热焊/现场焊
	...
	{
		if (vPMs[i].Mark != PM_SEW_LIVE && vPMs[i].Mark != PM_SEW_LRH)   //现场焊&&铝热焊缝
		{
			continue;
		}
		VINT vHeavyIndexes;
		for (int j = heaveIndex; j >= 0; --j)
		{
			int dstep = vPMs[i].Step2 - vHeavySteps[j].Start;   //位置标数据总步进 - Section.Start
		...
		if (vHeavyIndexes.size() == 1 && vHeavySteps[vHeavyIndexes[0]].Flag == 0)
		{
			AnalyseStep(DataA, blocks, vBA, vCRs, vHeavySteps[vHeavyIndexes[0]].Start, vWounds, vPMs, vER, vPMs2, vLCP, 30, i, btIndex);   //函数AnalyseStep()
			vHeavySteps[vHeavyIndexes[0]].Flag = 1;
		}
	}
#pragma endregion
...
#pragma region 处理其它多出波点
	...
	//根据vHeavySteps.End轨头15步进出波点数统计重排
...
#pragma region 处理人工标记的厂焊
	for (int i = vPMs2.size() - 1; i >= 0; --i)
	{
		...
		if (vPMs2[i].Mark != PM_SELFDEFINE || vPMs2[i].Data != 1 || vPMs2[i].IsOverlapped == 1)   //自定义
		{
			continue;
		}
		...
		CreateSection(vStepIndexes, 15, vsec);   //出波区间统计，组成一个个Section
		//只有一个
		if (vsec.size() == 1 && vsec[0].End - vsec[0].Start <= 20)
		{
			...
			for (int j = 0; j < 16; ++j)
			{
				GetCR(j, sstep, estep, blocks, vCRs[j], tcr[j]);   //函数GetCR()
...
#pragma region 分析连通域 AnalyseCR 伤损判定
...
#pragma region S4 填充人工标记伤损的信息
	for (int i = woundCount; i < vWounds.size(); ++i)
	{
		if (vWounds[i].Manual == 1)
		{
			...
			for (int j = 0; j < 16; ++j)
			{
				uint8_t bFind = GetCR(j, vWounds[i].Step2 - 1, vWounds[i].Step2 + 1, blocks, vCRs[j], tcr[j]);   //函数GetCR()
				for (int k = 0; k < tcr[j].size(); ++k)
				{
					count[j] += vCRs[j][tcr[j][k]].Region.size();
					GetCRInfo(vCRs[j][tcr[j][k]], DataA, blocks);   //函数GetCRInfo()
					AddWoundData(vWounds[i], vCRs[j][tcr[j][k]]);   //函数AddWoundData()
				}
			}
			...
			if (vWounds[i].Place == WP_HEAD_IN || vWounds[i].Type == W_JOINT || vWounds[i].Type == W_SEWLR || vWounds[i].Type == W_SEWCH)   //轨头内侧 接头伤损 铝热焊缝伤损 厂焊伤损
			{
				if (count[CH_C] + count[CH_c] >= 5)
				{
					vWounds[i].Place = WP_HEAD_MID;   //轨头中侧
				}
				else if (carType && channelAa > channelBb || !carType && channelAa < channelBb)
				{
					vWounds[i].Place = WP_HEAD_IN;   //轨头内侧
				}
				else
				{
					vWounds[i].Place = WP_HEAD_OUT;   //轨头外侧
				}
			}

			else if (vWounds[i].Place == WP_JAW_IN)   //轨颚内侧
			{
				if (carType && channelAa > channelBb || !carType && channelAa < channelBb)
				{
					vWounds[i].Place = WP_JAW_IN;   //轨颚内侧
				}
				else
				{
					vWounds[i].Place = WP_JAW_OUT;   //轨颚外侧
				}
			}
...
#pragma endregion
	...
	{
		int m = chParseOrder[order];   //{ CH_c, CH_a1, CH_A1, CH_b1, CH_B1, CH_C, CH_D, CH_E, CH_F, CH_G };
		VCR& vcr = vCRs[m];
...
#pragma region 回退校验（根据BackTotal过滤）
	if (backFlag == 0xFFFF)
	{
