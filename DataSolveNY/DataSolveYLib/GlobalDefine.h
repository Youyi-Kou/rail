...
extern VPM		g_vPMs;   //人工标记位置标数据
extern VPM		g_vPMs2;   //所有标记信息，包含焊缝，校验等
extern VWJ		g_vWounds;   //伤损数据
extern VBA		g_vBAs;   //回退数据
extern VER		g_vER;	 //单通道出波区域
extern VER		g_vER2;	  //所有通道出波区域
extern VLC		g_vLC;   //失耦信息
extern VLCP		g_vLCP;   //通道失耦信息
extern VLD		g_vLD;   //跳探信息

extern VPM		g_vPMsYOLO;   //所有标记信息（该位置标由YOLO判定）
