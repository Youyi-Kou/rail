...
int g_channelOffset[CH_N] = { 0 };   //CH_N A超通道数 12（将12个通道拼图参数均初始化为0）
...
F_HEAD	g_filehead;   // 文件头
SCPD*	g_StepPoints = NULL;   //每步进出波信息统计
VCO		g_vChannelOffsets;   //各通道参数
VBDB    g_vBlockHeads;
...
VINT		g_vReturnSteps;      //回退步进
VCR			g_vFLoseBig;   //出波连通域
...
VLCP		g_vLCP;   //通道失耦信息
VLD			g_vLD;   //跳探信息
VPM			g_vPMsYOLO;   //所有标记信息（该位置标由YOLO判定）
