...
uint32_t	ParsePosition(F_HEAD& head, VBDB& blocks, VCR* vCRs, CR& cr, int index, int16_t iFDesiredRow, uint8_t railType, VINT* t_cr, PM& pm, int& iAStepBig, int &iAStepSmall, VPM& vPMs, VPM& vPMs2)
{
	...
	uint32_t iBigCount = 0, iSmallCount = 0;
	iAStepBig = pf.iAStepBig;
	iAStepSmall = pf.iAStepSmall;
...
#pragma endregion
		...
		)//完整接头
	{
		if (pm.Percent >= 0.15)
		{
			...
			if (iSmallChannelwaveCountNoCc < 20)
			{
			...
			else if (iBigChannelwaveCountNoCc < 20)
			{
	...
	else if (pm.ChannelNum >= 5 && pm.Size >= 60 && iBigChannelwaveCountNoCc >= 40 && iBigChannelwaveCountNoCc >= iSmallChannelwaveCountNoCc * 2 && pf.maxSize >= 10 && (iLoseF && iLoseG) && pm.Num[CH_C] >= 10 && pf.maxSize >= 5)//开始半边接头
	{
		...
		{
			VINT vTempF;
			GetCR(CH_F, pf.iS1, iFDesiredRow - 3, pf.iS2 + 100, iFDesiredRow + 10, blocks, vCRs[CH_F], vTempF, -1, 500);
			if (vTempF.size() > 0)
			{
				pm.Mark = PM_JOINT_RIGHT;
				pm.IsHalf = 1;
	...
	*/
	...
	if (!IsJoint(pm.Mark) && pm.ChannelNum >= 3 && iLoseF && iLoseG)//ABc必须有
	{
	...
	//变坡点验证
	PM slope;
	int dist = GetLenWithSlope(cr.Step1, slope);
	if (dist < 300 && pm.ChannelNum >= 4 && pm.Percent >= 0.15)
	{
		...
		for (int i = CH_A1; i < CH_C; ++i)
		{
			chcount += GetCR(i, cr.Step1 - 300, 0, cr.Step2 + 300, 26, blocks, vCRs[i], t_ccc[i], t_cr[i], 5);
		}
	...
	int pMarkIndex = GetMarkedPositionIndex(cr.Step1 - 50, vPMs2, nullptr);
	//人工标记再次核对
	...
	if ((pm.Mark != 0 && pm.Mark != PM_JOINT_LEFT && pm.Mark != PM_JOINT_RIGHT || pm.Mark == PM_SEW_LRH && pm.ChannelNum >= 7 && pm.Length > 50) && pf.sum_ABC * pf.sum_abc > 0)
	{
		...
		//if (1.0 * count / pm.Size < 0.5)
		{
			...
			{
				distibute2[i] = Sum(distri, i - 15, i + 15, pf.iS2 - pf.iS1);
			}
			int maxValue = distibute2[0];
			int maxIndex = 0;
			...
			if (pm.Mark == PM_JOINT2 && iAStepBig - iAStepSmall >= 15)
			{
				...
				int sb = 0, se = 0;
				GetLoseStep(centerStep, blocks, sb, se);
				Pos temp2;
				if (sb > 0)
				{
