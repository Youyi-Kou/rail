...
bool	CanCombine(Connected_Region cr1, Connected_Region cr2, uint8_t channel)
{
	...
	else if (dist > 1 && cr1.Region.size() >= 5 && cr2.Region.size() >= 5)
	{
		return false;
	}
	if (dist == 1)
	...
	if (iOverlappedCount == 1 && vDist > 3 || iOverlappedCount > 1 && vDist >= 3 || iOverlappedCount >= 3)
	{
		return false;
	}
	...
	if (iOverlappedCount == 1 && hDist > 3 || iOverlappedCount > 1 && hDist >= 3 || iOverlappedCount >= 3)
	{
		return false;
	}
...
void    CombineDE(VCR &vCR, uint8_t channel)
{
	...
	for (int idx = 0; idx < n; ++idx)
	{
		...
		for (int i_LastFind = idx + 1; i_LastFind < n; ++i_LastFind)
		{
			...
			if (CanCombine(vCR[idx], vCR[i_LastFind], channel))
			{
				Combine(vCR[idx], vCR[i_LastFind]);
				vCR[i_LastFind].Region.clear();
				vCR[i_LastFind].IsUsed = 1;
...
void    CombineFG(VCR& vCR)
{
	...
	for (int idx = 0; idx < n; ++idx)
	{
		...
		for (int i_LastFind = idx + 1; i_LastFind < n; ++i_LastFind)
		{
			...
			if (vCR[i_LastFind].Step1 > vCR[idx].Step2 + 3)
			{
				break;
			}
			...
			if (GetDistance(vCR[i_LastFind], vCR[idx]) <= 3 &&
				(iFlag == 1 && dr1 <= 2 && dr2 <= 2 || iFlag == 0 && dr1 <= 2 && dr2 <= 2)
				)
			{
				Combine(vCR[idx], vCR[i_LastFind]);
				vCR[i_LastFind].Region.clear();
...
#pragma region AaBbCcDE
		...
		for (int j = 0; j < dataSize; ++j)
		{
			for (int step = 0; step < datas[j].vBStepDatas.size(); ++step)
			{
				...
				int maxRow = (g_iJawRow[railType] << 1);
...
#pragma region 分布参数计算
		...
		g_stepCount = g_iEndStep - g_iBeginStep + 1;

		g_StepPoints = new StepChannelPointDistrubute[g_stepCount];
		memset(g_StepPoints, 0, sizeof(SCPD) * g_stepCount);
		...
		for (int i = 0; i < CH_F; ++i)
		{
			for (int j = 0; j < wdata[i].size(); ++j)
			{
				...
				{
					g_StepPoints[wdata[i][j].step - g_iBeginStep].NumJaw++;
				}
			}
		}
		...
		for (int i = 0; i < g_stepCount; ++i)
		{
			for (int j = 0; j < CH_D; ++j)
			{
				if (g_StepPoints[i].Num[j] > 0)
				{
					...
					g_StepPoints[i].TotalHead += g_StepPoints[i].Num[j];
				}
			}
			for (int j = CH_D; j < CH_F; ++j)
			{
				...
				{
					g_StepPoints[i].ChannelNum++;
					g_StepPoints[i].TotalWaist += g_StepPoints[i].Num[j];
				}
			}
		}

		int * tempCountHead = new int[g_stepCount];
		int * tempCountWaist = new int[g_stepCount];
		int* tempCountJaw = new int[g_stepCount];
		for (int i = 0; i < g_stepCount; ++i)
		{
		...
		for (int i = 0; i < g_stepCount; ++i)
		{
			g_StepPoints[i].SumHead15 = Sum(tempCountHead, i - 15, i + 15, g_stepCount);
			g_StepPoints[i].SumWaist15 = Sum(tempCountWaist, i - 15, i + 15, g_stepCount);
			g_StepPoints[i].SumJaw15 = Sum(tempCountJaw, i - 15, i + 15, g_stepCount);
		}
...
#pragma endregion
		...
		std::vector<FullMeter> vJump;
		//首先计算每个步进底部FG出波行，防止底部突然出来一两个点，烦死人
		bool bFindF = true, bFindG = true, bLastFindF = true, bLastFindG = true;
		int16_t iFrow = datas[0].BlockHead.railH / 3, iGRow = iFrow;
		if (iFrow < g_iBottomRow[0] - 5)
		{
			iFrow = g_iBottomRow[datas[0].BlockHead.railType & 0x03];
			iGRow = iFrow;
		}
		int16_t iLastFrow = iFrow, iLastGrow = iGRow;
		for (int j = 0; j < dataSize; ++j)
		{
			int16_t iFRow1 = iFrow - 3, iGRow1 = iGRow - 3;
			for (int step = 0; step < datas[j].vBStepDatas.size(); ++step)
			{
				...
				{
					int16_t f2 = GetFRow2(b_step, iLastFrow);
					if (f2 > 0)
					{
						bFindF = true;
						b_step.isFindF = 1;
				...
				{
					int16_t g2 = GetGRow2(b_step, iLastGrow);
					if (g2 > 0)
					{
						bFindG = true;
						b_step.isFindG = 1;
		...
		int maxStep = datas[dataSize - 1].IndexL2 + datas[dataSize - 1].BlockHead.row;
		int minStep = datas[0].IndexL2;
		for (int i = 0; i < vJump.size(); ++i)
		{
			...
			if (vJump[i].Channel == CH_F)
			{
				...
				GetFRowRegion(datas, step - StepWindowSize, step, step, mapExistRows, vRows1, false);
				GetFRowRegion(datas, step, step + StepWindowSize, step, mapExistRows, vRows2, false);
				uint8_t maxValue;
				int maxIndex;
				GetMaxValueAndIndex(vRows1, maxValue, maxIndex);
				SetFRowRegion(datas, step - StepWindowSize, step, maxIndex, mapRising);
				GetMaxValueAndIndex(vRows2, maxValue, maxIndex);
				SetFRowRegion(datas, step, step + StepWindowSize, maxIndex, mapRising);
			}
		...
		for (int i = 0; i < dataSize; ++i)
		{
			...
			for (int step = 0; step < datas[i].vBStepDatas.size(); ++step)
			{
				...
				iFind[CH_G] = 0;
				uint8_t fRow[16] = { 0 };
				fRow[CH_F] = b_step.FRow - 2;
				fRow[CH_G] = b_step.GRow - 2;
				for (int row = 0; row < b_step.vRowDatas.size(); ++row)
				{
					...
					for (int m = 14; m < 16; ++m)
					{
						if (b_row.Point.Draw1 & bits[m] && b_row.Row < fRow[m])
						{
	...
	//S3.1 获取连通域
			...
			while (idx < nCount)
			{
				for (int i_Last = idx + 1; i_Last < nCount; ++i_Last)
				{
					if (GetDistance(vCRs[m][idx], vCRs[m][i_Last]) == 1)
					{
						...
						{
							vCRs[m][idx].Region.emplace_back(vCRs[m][i_Last].Region[j]);
						}
		...
		CombineDE(vCRs[CH_D], CH_D);
		CombineDE(vCRs[CH_E], CH_E);
		...
		for (int m = CH_D; m <= CH_E; ++m)
		{
			...
			{
				CR cr1, cr2;
				if (IsAngleCR(vCRs[m][i], datas, cr1, cr2))
				{
					vCRs[m][i] = cr1;
