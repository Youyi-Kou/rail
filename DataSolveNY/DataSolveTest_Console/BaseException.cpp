...
void CBaseException::OutputString(LPCTSTR lpszFormat, ...)
{
	...
	vsprintf(szBuf, (const char*)lpszFormat, args);
	va_end(args);

	std::string strErrorLogFile = g_strLogPath + "/error.log";
	FILE* pFileErr = fopen(strErrorLogFile.c_str(), "a");
	if (pFileErr == NULL)
	{
		return;
	}

	std::string st = GetCurrentTimeString();
	fprintf(pFileErr, "[%s]", st.c_str());
	fprintf(pFileErr, "%s\n", szBuf);
	fflush(pFileErr);
	fclose(pFileErr);

	WriteConsoleA(GetStdHandle(STD_OUTPUT_HANDLE), szBuf, strlen(szBuf), NULL, NULL);
}
...
{
	std::string strErrorDumpFile = g_strLogPath + "/exception.dump";
	//创建dump文件, 向文件写下当前程序崩溃相关信息
	HANDLE hFile = CreateFileA(strErrorDumpFile.c_str(), GENERIC_WRITE, 0, NULL, CREATE_ALWAYS, FILE_ATTRIBUTE_NORMAL, NULL);
