...
void AlgInit(char* strWorkPath, int len)   //3.4.算法初始化
//输入参数
//strWorkPath：工作路径，必须是绝对路径，’\’需要替换成’ / ’
//len : 路径字节数
{
	int sz = sizeof(StepChannelPointDistrubute);
...
void AnalyseBody(int iBeginBlock, int iEndBlock, VWJ& vWounds, VPM& vPMs, VPM& vPMs2, VPM& vPMsYOLO, VER& vER, VLCP& vLCP, int backFlag, int btIndex)
{
	...
	{
		g_vBlockHeads[i].FrameCountRead = GetBlockAFrames(g_pFileA, g_szFileA, g_vBlockHeads[i], vAFrames, iBeginFrame);
	}
...
int GetNextSplitBlock(int block1, int block2, int btIndex = -1)
{
	int block3 = block2;
	if (btIndex < 0)
	{
...
#pragma region SZT数据分析，提取回退，道岔，等人工标记数据
		...
		g_tas.Reset();
		g_tds.Reset();
		g_qualitydata.tas = &g_tas;
		g_qualitydata.tds = &g_tds;
		g_qualitydata.tss = &g_tss;
		g_qualitydata.tbs = &g_tbs;
		g_isQualityJudging = true;
		GetAlarmDefectSignData(&g_qualitydata);
		g_sztWoundCount = g_tds.DefectDataCount;
		g_sztAlarmCount = g_tas.AlarmDataCount;
...
#pragma region 文件内跳探
		if (g_vBlockHeads.size() > 0)
		{
			...
			for (int i = 1; i < g_vBlockHeads.size(); ++i)
			{
				...
				if (delt > 15)
				{
					ld.walk1 = g_vBlockHeads[i - 1].Walk;
					ld.walk2 = g_vBlockHeads[i].Walk;
					ld.step2 = g_vBlockHeads[i].IndexL2;
					ld.user = g_vBlockHeads[i - 1].BlockHead.user;
...
#pragma region 数据解析
		...
		for (int i = 0; i < g_vBAs.size(); ++i)
		{
			for (int j = g_vBAs[i].Pos0.Block; j < g_vBAs[i].Pos2.Block; ++j)
			{
...
#pragma region 基础数据统计

		VPM vJointHalf;
		for (int i = 0; i < g_vPMs.size(); ++i)
		{
			if (g_vPMs[i].Mark == PM_JOINT_LEFT || g_vPMs[i].Mark == PM_JOINT_RIGHT)
			{
...
//pInfo：文件头信息
{
	...
	try
	{
		...
		if (g_isPTData)
		{
			pInfo->gwd = g_filehead.deviceP2.TrackSet.sectionNum;
			pInfo->section = g_filehead.deviceP2.TrackSet.regionNum;
			pInfo->group = g_filehead.deviceP2.TrackSet.teamNum;
			pInfo->railno = g_filehead.deviceP2.TrackSet.lineNum;
			pInfo->PlyNo = g_filehead.deviceP2.TrackSet.plyNum;
			pInfo->StationNum = g_filehead.deviceP2.TrackSet.stationNum;
		}
		else
		{
		...
		printf("gwd = %d\n", pInfo->gwd);
		pInfo->direction = g_filehead.deviceP2.TrackSet.WalkWay;
		g_direction = g_filehead.deviceP2.TrackSet.WalkWay;
		pInfo->xingbie = g_filehead.deviceP2.TrackSet.lineWay;
		g_xingbie = g_filehead.deviceP2.TrackSet.lineWay;
		pInfo->xianbie = g_xianbie;

		strncpy(pInfo->instruNo, g_filehead.DeviceNum, 8);
		printf("instruNo = %s\n", pInfo->instruNo);

		strncpy(pInfo->dataV, g_filehead.DataVerS, 8);
		printf("dataV = %s\n", pInfo->dataV);
		strncpy(pInfo->softV, g_filehead.SoftVerS, 8);
		printf("softV = %s\n", pInfo->softV);
		strncpy(pInfo->fpgaV, g_filehead.FPGAVerS, 8);
		printf("fpgaV = %s\n", pInfo->fpgaV);
		...
		for (int i = 0; i < CH_N; ++i)
		{
			pInfo->Offset[i] = g_filehead.deviceP2.Place[i];
		}
		for (int i = 0; i < 4 * GA_NUM; ++i)
		{
			pInfo->Gate[i].isOn = g_filehead.deviceP2.Gate[i].isOn;
			if (g_isPTData == false)
			{
				pInfo->Gate[i].start = GetBigGatePixel(i / 5, g_filehead.deviceP2.Gate[i].start & 0x7FFF);
				pInfo->Gate[i].end = GetBigGatePixel(i / 5, g_filehead.deviceP2.Gate[i].end & 0x7FFF);
			}
			else
			{
				pInfo->Gate[i].start = 1.0 * (g_filehead.deviceP2.Gate[i].start & 0x7FFF);
				pInfo->Gate[i].end = 1.0 * (g_filehead.deviceP2.Gate[i].end);
			}
		}
		...
		g_filehead.step = 2.66;
		g_isFileOpened = 1;

		g_vBlockHeads.clear();
		GetBBlocks(g_pFileB, g_vBlockHeads, g_szFileB);
		...
		{
			uint8_t railType = g_vBlockHeads[i].BlockHead.railType & 0x03;
			for (int j = 0; j < 7; ++j)
			{
				uint16_t isOn = (0x8000 & g_vBlockHeads[i].BlockHead.door[2 * j]);
				if (g_isPTData)
				{
			...
			g_vBlockHeads[i].Walk2 = g_vBlockHeads[i].Walk = GetWD(g_vBlockHeads[i].BlockHead.walk);
			...
			if (!g_isPTData)
			{
				g_vBlockHeads[i].BlockHead.user = BCDToINT16(g_vBlockHeads[i].BlockHead.user);
				g_vBlockHeads[i].BlockHead.railNum = BCDToINT16(g_vBlockHeads[i].BlockHead.railNum);
			}
		}

		memset(pInfo->s_time, 0, 20);
		std::string sTime = GetTimeString(g_filehead.startD, g_filehead.startT);
		strncpy(pInfo->s_time, sTime.c_str(), sTime.length());
		printf("s_time = %s\n", pInfo->s_time);

		memset(pInfo->e_time, 0, 20);
		std::string eTime = GetTimeString(g_filehead.endD, g_filehead.endT);
		strncpy(pInfo->e_time, eTime.c_str(), eTime.length());
		printf("e_time = %s\n", pInfo->e_time);

		if (g_isPTData)
		{
			int timestack = g_vBlockHeads[g_vBlockHeads.size() - 1].BlockHead.time;
			uint16_t year = g_filehead.startD >> 16;
		...
		pInfo->s_mil = 0.000001 * g_filehead.startKm.mm + 0.001 * g_filehead.startKm.m + g_filehead.startKm.Km;
		pInfo->e_mil = 0.000001 * g_filehead.endKm.mm + 0.001 * g_filehead.endKm.m + g_filehead.endKm.Km;
		...
		*/
		...
		std::string newFileName = GetNewFileName(g_filehead, g_strFileName, pInfo->leftright, pInfo->xianbie);
...
//PrevCycleCount：历史周期个数
{
	...
	{
#endif // OUTPUT_EX
		std::string g_strTPA = g_strTPB.substr(0, g_strTPB.length() - 1);
		g_strTPA.append("A");
		g_pFileA = _fsopen(g_strTPA.c_str(), "rb", _SH_DENYWR);
		if (g_pFileA == NULL)
		{
		...
		SetFileHead(g_filehead);
		GetABlocks(g_pFileA, g_vBlockHeads, g_szFileA);
		for (int i = 0; i < g_vBlockHeads.size(); ++i)
		{
		...
		g_strWavePointFile = g_strLogPath + "/" + g_strFileNameWithoutExt + ".wd";
		g_pFileWavePoints = fopen(g_strWavePointFile.c_str(), "wb");
		if (g_pFileWavePoints == nullptr)
		{
			char sss[1000] = { 0 };
